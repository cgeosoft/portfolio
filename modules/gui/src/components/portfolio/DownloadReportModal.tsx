...
import type { PortfolioReport } from "portfolio-shared/portfolio";
import { api } from "../../api";
import { downloadBlob } from "../../environment";
import {
  X,
  Download,
  FileText,
...
export function DownloadReportModal({
  ...
  /** True inside the desktop window: files are saved to the Downloads folder. */
  ...
  useEffect(() => {
    if (isOpen && report) {
      ...
      api.getAppInfo().then((info) => setIsLocalClient(Boolean(info.isLocalClient))).catch(() => setIsLocalClient(false));
    }
  ...
  const handleExecuteDownload = async () => {
    ...
    try {
      const content: string | Blob = format === "markdown" ? generateReportMarkdown(report, portfolioName, hideValues) : await generateReportPdf(report, portfolioName, hideValues);

      if (isLocalClient) {
        // Desktop window: the service writes the file into ~/Downloads.
        let res;
        if (typeof content === "string") {
          res = await api.saveFile(cleanFileName, content);
        } else {
          const bytes = new Uint8Array(await content.arrayBuffer());
          let binary = "";
          for (let i = 0; i < bytes.byteLength; i++) binary += String.fromCharCode(bytes[i]!);
          res = await api.saveFile(cleanFileName, undefined, btoa(binary));
        }
        if (!res.success || !res.filePath) throw new Error(res.error || "Failed to save the report");
        setSavedFilePath(res.filePath);
        return;
      }

      // Browser (remote client): regular download.
      downloadBlob(cleanFileName, typeof content === "string" ? new Blob([content], { type: "text/markdown;charset=utf-8" }) : content);
  ...
  const handleRevealFile = useCallback(() => {
    if (savedFilePath && isLocalClient) {
      void api.revealFile(savedFilePath).catch((err: unknown) => {
        console.warn("revealFile failed:", err);
      });
    }
  }, [savedFilePath, isLocalClient]);
