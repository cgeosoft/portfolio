...
export default function App() {
  ...
  useEffect(() => {
    let cancelled = false;
    setUnauthorizedHandler(() => setAuthState((s) => (s === "ready" ? "locked" : s)));
    (async () => {
      setAuthState("loading");
      setAuthError(null);
      const reachable = await ensureRpcReady(15_000);
      if (cancelled) return;
      if (!reachable) {
        setAuthError("The Portfolio service did not answer.");
        setAuthState("error");
        return;
      }
      try {
  ...
  useEffect(() => {
    if (authState === "ready") loadPortfolios();
  }, [loadPortfolios, authState]);
