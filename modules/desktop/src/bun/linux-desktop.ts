...
export function setupLinuxDesktop(projectRoot?: string): void {
  ...
  try {
    ...
    const assetsDir = join(DESKTOP_DIR, "assets");
...
`;
    ...
    console.log("[desktop] Linux desktop development integration configured");
  } catch (error) {
...
export function setNativeWindowIcon(windowPtr: any, projectRoot?: string): boolean {
  ...
  try {
    ...
    const assetsDir = projectRoot ? join(projectRoot, "modules/desktop/assets") : join(DESKTOP_DIR, "assets");
    const candidateIcons = [
      join(dirname(process.execPath), "../Resources/app/app-icon.png"),
      join(dirname(process.execPath), "../Resources/appIcon.png"),
      join(assetsDir, "app-icon-256x256.png"),
