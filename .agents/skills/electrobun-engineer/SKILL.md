...
- Files: `modules/desktop/src/bun/index.ts` (shell: spawns `service/main.js`, waits for `/api/health`, loads `http://127.0.0.1:5130`, window state, external links), `modules/desktop/scripts/stage.ts` (builds GUI and service into `stage/`, bakes `PORTFOLIO_VERSION` and `POSTHOG_API_KEY`), `modules/desktop/electrobun.config.ts`. Service side: `modules/service/src/main.ts` (Bun.serve), `src/http/routes.ts` (every route), `src/config.ts` (`config` table, `DesktopConfig`; a new field needs a `DEFAULT_CONFIG` entry in `config.ts` and the type in `modules/shared/src/config-types.ts`), `src/logger.ts` (`appLogger` with `logStep()` / `startTimer()`; metadata goes in `data`), `src/db/database.ts` (WAL, foreign keys, additive migrations via `PRAGMA table_info`; one repo module per table), `src/services/metrics/` (`runtime.ts` spawns the engine process, `registry.ts` merges built-ins with URL installs), `src/services/host-settings.ts` (remote-access switch), `src/services/auth.ts` (PIN, sessions).
