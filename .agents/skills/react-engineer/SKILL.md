...
- React 19, Chart.js 4, Lucide. Separate `src/views/tsconfig.json`; no `bun` or `node:*` imports.
- The client is `Electroview.defineRPC<PortfolioRPC>` in `src/views/src/rpc.ts`: all backend access is `rpc.request.<name>(payload)`. Call `ensureRpcReady()` before initial requests; do not break the `forceFallbackToNativeBridge()` WebKitGTK failover. Read `src/shared/rpc-types.ts` before adding a call.
...
- Metrics: the webview never formats a metric itself; call `evaluatePortfolioMetrics` and render through `displayMetric()`. Selection edits go through the helpers in `src/shared/metrics.ts` (capacity 4 large, 6 compact).
- Use the `cx-card` surface and `input.css` tokens; run `bun run build:css` after adding utility classes.
