...
- Tailwind CSS 3.4 with `tailwind.config.js`; tokens are CSS custom properties in `src/views/src/input.css`. Compiled output `src/views/style.css` via `bun run build:css` (`watch:css` while developing).
...
- Utility classes in `.tsx`, CSS in `input.css`; `bun run build:css` after any class or CSS change.
